  ...
  module Project
    class  Prepare < BaseInteraction
      ...
      def execute
        ...
        if remote_staging_branch_exists
          compare_against_branch(staging_branch, "origin/#{staging_branch}")
        end
      ...
      def compare_against_branch(staging_branch, other_branch)
        ...
        if ahead.positive? or ahead.zero?
      ...
      def prompt_stash
        if prompt.yes?(PROMPT_STASH % [project.name])
